...
jQuery(function($){
	...
	grid_modal_get_elements = function(obj){
		...
		var object = {
			...
			categories = obj.trigger.data('categories').split(';');
			
		for( var i = 0; i < categories.length; i++ ){
			if( !object.category[ categories[i] ] ){
		...
		$('.insert-item').each(function(k,v){
			...
			for( var i = 0; i < categories.length; i++ ){
				if( object.category[ categories[i] ] ){
					object.category[ categories[i] ].element[ $(this).data('elementSlug') ] = $(this).data('elementType');
				}
			}
		})
		return object;
	...
	grid_field_form_check = function(obj){
		...
		var modal_config = $(obj).closest('form').formJSON();
		if( modal_config._invalid_fields.length ){
			for( var i = 0; i < modal_config._invalid_fields.length; i++){
				modal_config._invalid_fields[i].addClass('invalid');
				if( i === 0 ){
					modal_config._invalid_fields[i].focus();
				}
			}
			return false;
		}
