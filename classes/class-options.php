...
/**
 * Caldera_Forms_Options class.
 *
 * @package Caldera_Forms
 * @author  David <david@digilab.co.za>
 */
class Caldera_Forms_Options {

	public static $option_name = 'caldera_forms';

	/**
	 * Create a new caldera_forms.
	 *
	 * @since 0.0.1
	 *
	 * @param string $name Name for caldera_forms.
	 * @param string $slug Slug for caldera_forms.
	 *
	 * @return array|void Config array for new caldera_forms if it exists. Void if not.
	 */
	 ...
	 * @since 0.0.1
	 *
	 * @param string $id caldera_forms ID
	 *
	 * @return bool|array caldera_forms config or false if not found.
	 */
	public static function get_single( $id ) {
		$option_name = self::item_option_name( $id );
		$caldera_forms = get_option( $option_name, false );
		
		// try slug based lookup
		if( false === $caldera_forms ){
			$registry = self::get_registry();
		...
		/**
		 * Filter a caldera_forms config before returning
		 *
		 * @param array $caldera_forms The config to be returned
		 * @param string $option_name The name of the option it was stored in.
		 *
		 * @since 0.0.1
		 */
		return apply_filters( 'caldera_forms_get_single', $caldera_forms, $option_name );

	}

	/**
	 * Get the registry of caldera_forms.
	 *
	 * @since 0.0.1
	 *
	 * @return mixed|void
	...
	public static function get_registry() {
		 ...
		 * @since 0.0.1
		 */
		return apply_filters( 'caldera_forms_get_registry', $registry );
	 ...
	 *
	 * @param string $id caldera_forms ID
	 * @param array $config caldera_forms config
	 */
	...
	protected static function create_unique_id() {
		$slug_parts = explode( '_', 'caldera_forms' );
		$slug = '';
